...
CUDD::BDD BuchiSolver::CPre_agent(const CUDD::BDD& W_states) const {
    ...
    CUDD::BDD T;
    // reuse cached VectorCompose result if W is unchanged
    if (has_cached_vectorcompose_ && (cached_vectorcompose_W_ == W)) {
        T = cached_vectorcompose_T_;
    } else {
        T = W.VectorCompose(transition_compose_vector_);
        cached_vectorcompose_W_ = W;
        cached_vectorcompose_T_ = T;
        has_cached_vectorcompose_ = true;
    }
